...
msg="Before doing anything else, restate in one or two terse lines what you understand this request to be asking, so the user can confirm you read it right. Do NOT start executing yet. If the request has more than one plausible interpretation, present the options via AskUserQuestion (multiple choice) or ask focused follow-up questions — never pick an interpretation on the user's behalf. For any non-trivial or multi-step ask, after restating use the task tool (TaskCreate) to break the work into prioritized tasks, tag each with a complexity estimate (S/M/L plus a rough time or token size so the user can judge whether to proceed), show the list, and get the user's approval — pausing for an explicit OK before any destructive or irreversible step. Begin the actual work only after the user confirms. EXCEPTION: if this prompt is itself a direct confirmation or answer to your own previous clarifying question, treat intent as confirmed and proceed without re-echoing."
