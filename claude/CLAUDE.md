...
`confirm-intent.sh` (UserPromptSubmit) injects a reminder on every submitted prompt: before doing anything, restate in one or two terse lines what the request is understood to be asking, and do NOT start executing yet. If the prompt has more than one plausible interpretation, present the options via `AskUserQuestion` (multiple choice) or ask focused follow-up questions — never assume on the user's behalf. Begin the work only after the user confirms. For any non-trivial / multi-step ask it also requires breaking the work into tasks via `TaskCreate` ordered by sensible default priority (prerequisites first, then value/impact), each tagged with a complexity + estimated processing time (S/M/L + rough minutes/token size), shown for approval, then worked **one at a time** — pausing before each task to state its estimate and get a go-ahead — at each check-in offering the choice to continue one task at a time or run all remaining tasks without further pauses (autopilot) — and always pausing before any destructive/irreversible step regardless. A new request that arrives mid-queue is folded into the existing task list and re-prioritized, never interrupting in-flight work. The task queue is durable across interruptions and `/clear` — resume incomplete tasks rather than restarting, and never delete a task unless the user explicitly asks. Exception: a prompt that is itself a direct confirmation/answer to a prior clarifying question is treated as confirmed and proceeds without re-echoing. Skips slash-commands (`/foo`) and bang-shell (`!cmd`) lines. Override `CLAUDE_INTENT_CONFIRM_DISABLED=1`.
